...
namespace QueueSystem.Services
{
    ...
    public class SqlDoctor : IDoctorsData
    {
        ...
        public IEnumerable<Doctor> GetDoctorByClinic(int id, string floor)
        {
            ...
            string CommandText = $"SELECT [Name], [code], ISNULL([RecWait], 0 ) as RecWait, ISNULL([DocWait], 0 ) as DocWait, ISNULL([Seeing], 0) as Seeing, ISNULL([Saw], 0) as Saw FROM [ALDar_Hospital].[dbo].[{f}] Where [Spec] = cast({id} as varchar(50)) ORDER BY [code]";
            ...
            for (int i = 0; i < myDataTable.Rows.Count; i++)
            {
                ...
                list.Add(new Doctor
                {
                    ...
                    RecWait = Convert.ToInt32(myDataTable.Rows[i]["RecWait"]),
                    DocWait = Convert.ToInt32(myDataTable.Rows[i]["DocWait"]),
                    Seeing = Convert.ToInt32(myDataTable.Rows[i]["Seeing"]),
                    Saw = Convert.ToInt32(myDataTable.Rows[i]["Saw"]),
                    Code = (myDataTable.Rows[i]["code"]).ToString(),
                    InDuty = (listInt[i] == 1) ? true : false
                });
            }
